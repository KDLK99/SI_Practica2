...
def access_cve_api():
    ...
    for i in range(len(body_dict)):
        try:
            list_strings.append(f"{body_dict[i]['vulnerabilities'][0]['cve']}: {body_dict[i]['vulnerabilities'][0]['cwe']['name']}\n"
                  f"{body_dict[i]['vulnerabilities'][0]['notes'][1]['text']}\n")
        except:
            try:
                list_strings.append(f"{body_dict[i]['cveMetadata']['cveId']}: {body_dict[i]['containers']['adp'][0]['problemTypes'][0]['descriptions'][0]['description']}\n")
            except:
                try:
